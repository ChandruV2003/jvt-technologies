...
def rejection_reasons(payload: dict[str, Any]) -> list[str]:
    ...
    if not payload.get("follow_up_stage") and not payload.get("follow_up_parent_stem"):
...
def main() -> None:
    ...
    for path in sorted(REVIEW.glob("*.json"), key=lambda item: (item.stat().st_mtime, item.name)):
        ...
        if not payload:
            continue
        if not (payload.get("follow_up_stage") or payload.get("follow_up_parent_stem")):
            continue
        reasons = rejection_reasons(payload)
        model_review: dict[str, Any] | None = None
        approval_reason = "strict follow-up quality pass"
        if has_existing_quality_hold(payload):
            ...
            if model_review.get("approved"):
                reasons = []
                approval_reason = f"model-assisted follow-up quality pass: {model_review.get('reason')}"
        item = {
            "stem": path.stem,
    ...
    print(json.dumps({"report_path": str(report_path), "approved_count": len(approved), "held_count": len(held), "approved_stems": [item["stem"] for item in approved]}, indent=2))
