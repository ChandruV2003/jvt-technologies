#!/bin/zsh
set -euo pipefail

ROOT="/Users/c.s.d.v.r.s./Developer/Control-Host/JVT-Technologies"
STATE_DIR="$ROOT/ops/agent-control/state"
HOST="${JVT_MLX_BIND_HOST:-127.0.0.1}"
PORT="${JVT_MLX_PORT:-11435}"
MODEL="${JVT_MLX_MODEL_PATH:-/Users/c.s.d.v.r.s./.cache/huggingface/hub/models--mlx-community--Qwen3-8B-4bit/snapshots/545dc4251c05440727734bcd94334791f6ab0192}"
MODEL_ID="${JVT_MLX_MODEL:-mlx-community/Qwen3-8B-4bit}"
BASE_URL="http://$HOST:$PORT"

mkdir -p "$STATE_DIR"

python3 - "$STATE_DIR/latest-m4-mlx-model-server.json" "$HOST" "$PORT" "$MODEL" "$MODEL_ID" <<'PY'
import json
import sys
from datetime import datetime, timezone

path, host, port, model_path, model_id = sys.argv[1:6]
payload = {
    "generated_at": datetime.now(timezone.utc).isoformat(timespec="seconds"),
    "status": "starting",
    "message": "M4 MLX model server process is starting under launchd.",
    "base_url": f"http://{host}:{port}",
    "model_path": model_path,
    "model_id": model_id,
}
open(path, "w", encoding="utf-8").write(json.dumps(payload, indent=2) + "\n")
PY

(
  for _ in {1..45}; do
    if curl -fsS --max-time 3 "$BASE_URL/health" >/dev/null 2>&1; then
      python3 - "$STATE_DIR/latest-m4-mlx-model-server.json" "$BASE_URL" "$MODEL" "$MODEL_ID" <<'PY'
import json
import sys
from datetime import datetime, timezone

path, base_url, model_path, model_id = sys.argv[1:5]
payload = {
    "generated_at": datetime.now(timezone.utc).isoformat(timespec="seconds"),
    "status": "ok",
    "message": "M4 MLX model server is healthy under launchd foreground runner.",
    "base_url": base_url,
    "model_path": model_path,
    "model_id": model_id,
}
open(path, "w", encoding="utf-8").write(json.dumps(payload, indent=2) + "\n")
PY
      exit 0
    fi
    sleep 2
  done
  python3 - "$STATE_DIR/latest-m4-mlx-model-server.json" "$BASE_URL" "$MODEL" "$MODEL_ID" <<'PY'
import json
import sys
from datetime import datetime, timezone

path, base_url, model_path, model_id = sys.argv[1:5]
payload = {
    "generated_at": datetime.now(timezone.utc).isoformat(timespec="seconds"),
    "status": "error",
    "message": "M4 MLX model server did not report healthy within the foreground runner window.",
    "base_url": base_url,
    "model_path": model_path,
    "model_id": model_id,
}
open(path, "w", encoding="utf-8").write(json.dumps(payload, indent=2) + "\n")
PY
) &

exec python3 -m mlx_lm server \
  --model "$MODEL" \
  --host "$HOST" \
  --port "$PORT" \
  --max-tokens "${JVT_MLX_MAX_TOKENS:-220}" \
  --temp "${JVT_MLX_TEMP:-0.0}" \
  --chat-template-args '{"enable_thinking":false}'
