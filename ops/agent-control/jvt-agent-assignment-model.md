...
## JVT E.G.G. - Execution, Governance & Growth

`egg_agent.py` is JVT's goal-aware operating-intelligence generator. It reads the
...
E.G.G. is not an external-action agent. It does not send, approve, spend, trade,
...
- `com.jvt.egg-agent` runs E.G.G. every 15 minutes.
- `growth_ops_checkin.py` also runs E.G.G. before invoking the local runner.
- `ai_director.py` can seed `egg_task_generator` when E.G.G. has not produced
  state yet.
